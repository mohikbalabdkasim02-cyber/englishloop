...
function readNamedKey(envName:string,fallbackName:string){const raw=Deno.env.get(envName);if(raw){try{const parsed=JSON.parse(raw) as Record<string,string>;if(parsed.default)return parsed.default;const first=Object.values(parsed)[0];if(first)return first}catch{}}return Deno.env.get(fallbackName)??""}
Deno.serve(async(req:Request)=>{
 ...
 const supabaseUrl=Deno.env.get("SUPABASE_URL")??"";const publishableKey=readNamedKey("SUPABASE_PUBLISHABLE_KEYS","SUPABASE_ANON_KEY");const secretKey=readNamedKey("SUPABASE_SECRET_KEYS","SUPABASE_SERVICE_ROLE_KEY");const authorization=req.headers.get("Authorization");
 if(!supabaseUrl||!publishableKey||!secretKey||!authorization)return json({error:"Server authentication is not configured."},500);
 ...
 const {data:caller}=await userClient.from("profiles").select("role").eq("id",user.id).single();if(!caller||!["teacher","admin"].includes(caller.role))return json({error:"Only teachers or admins can manage student accounts."},403);
 ...
 const {error:profileError}=await admin.from("profiles").update({class_id:classId,cefr_level:level,name,username}).eq("id",created.user.id);if(profileError){await admin.auth.admin.deleteUser(created.user.id);return json({error:"Student account could not be linked to the class."},500)}
 return json({ok:true,student:{id:created.user.id,name,username,cefr_level:level,class_id:classId}},201)
});
